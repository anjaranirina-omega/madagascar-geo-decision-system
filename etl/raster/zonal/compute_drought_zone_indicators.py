...
def upsert_drought_indicator(payload: dict, token: str | None = None):
...
def process_table(zone_type: str, table_name: str, raster_layer_id: str | None, token: str | None = None):
    ...
    with rasterio.open(POPULATION_RASTER) as pop_src, rasterio.open(
        ...
        for idx, row in zones.iterrows():
            ...
            upsert_drought_indicator(payload, token=token)
...
def main():
    parser = argparse.ArgumentParser(description="Calcul des indicateurs zonaux du risque sécheresse.")
    ...
    with engine.begin() as conn:
        ...
        raster_layer_id = get_active_raster_layer_id(conn, "DROUGHT_RISK_INDEX")
        if raster_layer_id:
            print(f"  Raster layer ID (DROUGHT_RISK_INDEX) : {raster_layer_id}")
        else:
            print("  Avertissement : aucun raster_layer_id actif trouvé pour DROUGHT_RISK_INDEX")
