...
def upsert_cyclone_indicator(payload: dict, token: str | None = None):
...
def process_table(zone_type: str, table_name: str, raster_layer_id: str | None, token: str | None = None):
    print(f"\nTraitement cyclone {zone_type} depuis table {table_name}")
    ...
    with rasterio.open(POPULATION_RASTER) as pop_src, rasterio.open(
        CYCLONE_RISK_RASTER
    ) as cyclone_risk_src, rasterio.open(CYCLONE_HAZARD_RASTER) as cyclone_hazard_src:
        for idx, row in zones.iterrows():
            geometry = row["geom"]

            population_values = mask_values(pop_src, geometry)
            cyclone_risk_values = mask_values(cyclone_risk_src, geometry)
            cyclone_hazard_values = mask_values(cyclone_hazard_src, geometry)
            ...
            risk_mean = (
                float(cyclone_risk_values.mean()) if cyclone_risk_values.size else None
            )
            risk_max = (
                float(cyclone_risk_values.max()) if cyclone_risk_values.size else None
            )
            hazard_mean = (
                float(cyclone_hazard_values.mean())
                if cyclone_hazard_values.size
                else None
            )
            ...
            upsert_cyclone_indicator(payload, token=token)
...
def main():
    parser = argparse.ArgumentParser(description="Calcul des indicateurs zonaux du risque cyclonique.")
    ...
    required_files = [
        ...
        ("Cyclone risk raster", CYCLONE_RISK_RASTER),
        ("Cyclone hazard raster", CYCLONE_HAZARD_RASTER),
    ]
    ...
    print(f"Cyclone risk raster : {CYCLONE_RISK_RASTER}")
    print(f"Cyclone hazard raster : {CYCLONE_HAZARD_RASTER}")
    ...
    with engine.begin() as conn:
        ...
        raster_layer_id = get_active_raster_layer_id(conn, "CYCLONE_RISK_INDEX")
        if raster_layer_id:
            print(f"  Raster layer ID (CYCLONE_RISK_INDEX) : {raster_layer_id}")
        else:
            print("  Avertissement : aucun raster_layer_id actif trouvé pour CYCLONE_RISK_INDEX")

        for zone_type, table_name in TABLES:
            process_table(zone_type, table_name, raster_layer_id, token=token_to_use)

    print("Calcul des indicateurs zonaux cyclone terminé.")
