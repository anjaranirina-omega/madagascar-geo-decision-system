...
def compute_area_km2(geometry):
    ...
    # EPSG:6933 = projection mondiale équivalente en surface.
    # Plus adaptée qu'EPSG:3857 pour les statistiques de surface.
    gdf = gpd.GeoSeries([geom], crs="EPSG:4326").to_crs(epsg=6933)

    return float(gdf.area.iloc[0] / 1_000_000)


def upsert_flood_indicator(payload: dict, token: str | None = None):
...
def process_table(zone_type: str, table_name: str, raster_layer_id: str | None, token: str | None = None):
    ...
    with rasterio.open(POPULATION_RASTER) as pop_src, rasterio.open(
        FLOOD_RISK_RASTER
    ) as flood_risk_src, rasterio.open(FLOOD_HAZARD_RASTER) as flood_hazard_src:
        for idx, row in zones.iterrows():
            geometry = row["geom"]

            population_values = mask_values(pop_src, geometry)
            flood_risk_values = mask_values(flood_risk_src, geometry)
            flood_hazard_values = mask_values(flood_hazard_src, geometry)
            ...
            risk_mean = (
                float(flood_risk_values.mean()) if flood_risk_values.size else None
            )
            risk_max = (
                float(flood_risk_values.max()) if flood_risk_values.size else None
            )
            hazard_mean = (
                float(flood_hazard_values.mean())
                if flood_hazard_values.size
                else None
            )
            ...
            upsert_flood_indicator(payload, token=token)
...
def main():
    parser = argparse.ArgumentParser(description="Calcul des indicateurs zonaux du risque inondation.")
    ...
    required_files = [
        ...
        ("Flood risk raster", FLOOD_RISK_RASTER),
        ("Flood hazard raster", FLOOD_HAZARD_RASTER),
    ]
    ...
    print(f"Flood risk raster : {FLOOD_RISK_RASTER}")
